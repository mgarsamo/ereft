...
def oauth_redirect_handler(request):
    ...
    try:
        ...
        # Process the OAuth code to get user info and create/login user
        try:
            # Google OAuth configuration
            GOOGLE_CLIENT_ID = '91486871350-79fvub6490473eofjpu1jjlhncuiua44.apps.googleusercontent.com'
            GOOGLE_CLIENT_SECRET = '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'
            ...
            deep_link_url = f"ereft://oauth?token={token.key}&user_id={user.id}&email={email}&first_name={first_name}&last_name={last_name}&google_id={google_id}"
            ...
            <head>
                ...
                <style>
                    ...
                    .redirecting {{ color: #666; margin: 20px 0; }}
            ...
            <body>
                ...
                <p class="redirecting">Redirecting to Ereft app...</p>
                <p class="manual-link">
                    <a href="{deep_link_url}">Click here if not redirected automatically</a>
                </p>
                <script>
            ...
            print(f"🔐 User creation/login failed: {str(e)}")
            return JsonResponse({
                'error': f'User creation/login failed: {str(e)}'
            }, status=500)
...
urlpatterns = [
    ...
    path('google-oauth/', google_oauth_view, name='google_oauth'),  # Google OAuth endpoint
